...
#
# The engine, the jsonic grammar this plugin layers on, the strict-JSON
# core jsonic itself layers on, and the fixture runner are all PATH
...
for SIBLING in parser jsonic json support; do
...
# above while leaving the committed lock stale: cargo then regenerates it
# in the runner and everything goes green.
#
# So the whole resolution is compared, before and after cargo runs, with
# one exemption: the recorded version of each sibling crate. Those entries
# legitimately move whenever the sibling checkouts do (the engine, the
# jsonic grammar, the strict-JSON core and the fixture runner are all
# resolved from a sibling's MAIN), and exempting exactly them is what
...
# NOT `--locked`, deliberately, and this is the one place a plugin gate
...
# resolved from checkouts of MAIN. So the day parser, jsonic, json or
# support bumps its crate version, `--locked` here fails with "cannot
# update the lock file" on every pull request in this repo, including
# ones that touch no Rust at all: a red build caused by another
# repository's release. The engine repo has no path dependency of its
...
# The exempted sibling versions may still have moved, and cargo wrote
# them into the lock. Put the lock back so a green gate leaves the tree
# exactly as it found it, whatever the siblings did: updating the
# committed lock is a deliberate cargo run and commit, never a side
