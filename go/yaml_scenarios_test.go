...
// A `baseline` helper used to live here. It mirrored the TS pattern
//
//	try { result = y(src) } catch (e) { result = 'ERROR' }
//	assert.ok(result != null)
//
// which accepted a parse error OR any non-nil value — i.e. it could not fail
// for any input, in either runtime, so no behaviour change could ever move
// it. Both sides now assert the exact observed value instead.

// ===== BLOCK MAPPINGS =====

// TS: colon-space-required — no space after the colon, so YAML reads the
// whole line as one plain scalar.
...
// value stays a plain string. Pinned exactly rather than merely "not nil".
...
// tab as indentation, so this document should be REJECTED. It is not: the tab
